...
public class Main {
    ...
    public static void CreateAbonnement() throws Exception {
        ...
        if (!statut.equals("ACTIVE") && !statut.equals("SUSPENDU") && !statut.equals("RESILIE")) {
            LoggerService.log("Statut d'abonnement invalide: " + statut);
        ...
        if (type.equals("A")) {
            System.out.println("Enter la duree du contract : ");
            int dure = sc.nextInt();
            if (dure <= 0) {
                throw new Exception("La duree d'engagement doit etre positive !");
            }
            sc.nextLine();
    ...
    public static void UpdateAbonnement() throws Exception {
        ...
        if (type.equals("A")) {
            System.out.println("Enter la duree du contract (Mois) : ");
            int dure = sc.nextInt();
            if (dure <= 0) {
                throw new Exception("La duree d'engagement doit etre positive !");
            }
    ...
    public static void EnregistrerPaiement() throws Exception {
        ...
        LocalDate prochaineEcheance = abonnementService.genererEcheances(abonnement).stream()
                ...
                .findFirst()
                .orElseThrow(() -> new Exception("Aucune Echeance Detecte (Abonnement Termine)"));
        ...
        if (datePaiement.isBefore(abonnement.getDateDebut())) {
            LoggerService.log("Date de paiement invalide (avant date debut)");
            throw new Exception("La date de paiement ne peut pas etre avant la date de début !");
        }
        ...
        String statutStr = sc.nextLine().toUpperCase();
        if (!statutStr.equals("PAYE") && !statutStr.equals("NON_PAYE") && !statutStr.equals("EN_RETARD")) {
            throw new Exception("Statut de paiement invalide !");
        }
        StatutPaiement statut = StatutPaiement.valueOf(statutStr);
        ...
        LoggerService.log("Paiement enregistre pour abonnement ID " + abonnementId + " | Montant: " + abonnement.getMontantMensuel() + " | Statut: " + statut);
    ...
    public static void ModifierPaiement() throws Exception {
        ...
        if (!datePaiementStr.isEmpty()) {
            LocalDate newDate = LocalDate.parse(datePaiementStr);
            if (newDate.isBefore(paiement.getDateEcheance())) {
                LoggerService.log("Date de paiement modifiée invalide: " + newDate);
                throw new Exception("La nouvelle date de paiement ne peut pas être avant l' echeance !");
            }
            paiement.setDatePaiement(newDate);
        }
        ...
        if (!statutStr.isEmpty()) {
            if (!statutStr.equalsIgnoreCase("PAYE") && !statutStr.equalsIgnoreCase("NON_PAYE") && !statutStr.equalsIgnoreCase("EN_RETARD")) {
                LoggerService.log("Statut de paiement modifié invalide: " + statutStr);
                throw new Exception("Statut de paiement invalide !");
            }
